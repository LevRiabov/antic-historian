...
| **D2** | ~~Embedding model~~ **DECIDED 2026-06-12** ([ADR-002](docs/adr/002-d2-embeddings.md)) | — | **qwen3-embedding-8b hosted** (OpenRouter pinned to Nebius, $0.01/M, 1024d MRL-truncated): +18.0 recall@5 over the 0.6b incumbent (35.2%→53.2%), synonym +41.7 — the embedder, not enrichment, was the binding constraint. Local 0.6b stays as the measured fallback; small-local shortlist arms skipped after the gap made them implausible (rationale in ADR). Decided by ablation on golden set v2.0; ~$0.36 total. |
| **D3** | ~~Vector store~~ **DECIDED 2026-06-14** | — | **Postgres + pgvector** (Docker locally, Neon free tier in prod — one store for vectors and relational data, full local/cloud parity). The Phase-4.3 hybrid ablation confirmed the default: Postgres FTS+RRF was byte-identical to dense (the 8B embedder subsumes BM25), so the Qdrant "native fusion" challenger never triggered; the FTS column stays in-schema (idle) for a possible keyword-filter arm. See [docs/vector-stores.md](docs/vector-stores.md) + eval-log 2026-06-14. |
| **D4** | ~~Frontend: Next.js vs Vite SPA~~ **DECIDED 2026-06-17** ([ADR-004](docs/adr/004-d4-frontend.md)) | — | **Vite + React SPA** (TS strict), served as a static bundle by **nginx in a multi-stage Docker image**. Next rejected: backend is FastAPI either way (no API routes needed) and the demo is interactive (SSR first-paint is a weak win), so Next's Node server is a moving part for zero capability we need. Browser hits **one origin** — a `/api` prefix the Vite dev server + nginx both rewrite to FastAPI, so **no CORS**. Judgment call, not a golden-set ablation (no quality rides on it). |
| **D5** | ~~LLM lineup (chat, agent, judge, cheap-tier)~~ **DECIDED 2026-06-16** ([ADR-003](docs/adr/003-d5-llm-lineup.md)) | — | **Agent = deepseek/deepseek-v4-pro** (OpenRouter, grammar-ReAct, the KEEP after gemma); **judge = split** — kimi-k2.6 (refusal/faithfulness/completeness) + qwen3.7-max (attribution only, the noise-limited rubric). Retrieval ships **rerank-free** (`dense-ctx-v1`; the Phase-4.2 cohere-pro KEEP was overturned — marginal+paid over the 8B embedder). Still open, decided in Phase 6: the single-shot default model, the cheap/demo tier (6.5 routing ablation), and the fallback order (6.4). Abstraction keeps it all reversible (ADR-001). |
...
### Phase 7 — Frontend + deploy → ~~Gate D4~~ **D4 DECIDED** ([ADR-004](docs/adr/004-d4-frontend.md))
TS frontend (chat with inline expandable citations, evals tab, cost display, how-it-works page, refusal-trigger demo prompt). **Scaffold landed 2026-06-17**: Vite+React+TS SPA, Tailwind v4, react-router, typed SSE client (`frontend/src/lib/`), Dockerized (nginx + `/api` proxy) — route stubs, pages next. Deploy: frontend on Vercel/CF Pages or the nginx image, API on a free/cheap CPU tier (Fly.io / Render / HF Spaces).
