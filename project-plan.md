...
|---|---|---|---|
| **D1** | RAG/agent framework: **LlamaIndex vs LangChain+LangGraph vs Haystack** | End of Phase 0 | 1–2 day spike each on a toy pipeline (ingest → retrieve → cite → stream). Criteria: control/transparency, advanced-retrieval support (RAPTOR/GraphRAG/rerankers built in?), streaming + citation ergonomics, market recognition, docs quality. Working hypothesis: **LlamaIndex for the RAG layer** (best advanced-retrieval coverage: RAPTOR pack, PropertyGraphIndex), optionally **LangGraph for agent orchestration** later — both names carry hiring weight. |
